import{_ as n,c as s,o as a,a4 as p}from"./chunks/framework.BG61BEI0.js";const g=JSON.parse('{"title":"一、synchronized","description":"","frontmatter":{},"headers":[],"relativePath":"01-Dev/01.01-Java/JUC/多线程.md","filePath":"01-Dev/01.01-Java/JUC/多线程.md","lastUpdated":1713954705000}'),l={name:"01-Dev/01.01-Java/JUC/多线程.md"},e=p(`<h2 id="锁升级" tabindex="-1">锁升级 <a class="header-anchor" href="#锁升级" aria-label="Permalink to &quot;锁升级&quot;">​</a></h2><h2 id="threadlocal" tabindex="-1">ThreadLocal <a class="header-anchor" href="#threadlocal" aria-label="Permalink to &quot;ThreadLocal&quot;">​</a></h2><h2 id="cas" tabindex="-1">CAS <a class="header-anchor" href="#cas" aria-label="Permalink to &quot;CAS&quot;">​</a></h2><p>比较并替换, 它能够保证 单个方法是原子的 但是不能保证你连续的多个它的方法都是原子的</p><p>CAS会带来 一个ABA问题</p><p>举个例子 线程1 : 想 把 A 改成B 刚进去 CPU调度就把执行权拿走了, 此时 线程挂起 线程2: 它吧A改成了B 又改回A 恰巧 这个过程 在 线程1 线程挂起的时候 完成了 此时 线程1 不知道 A已经变成了B又变成了A, 所以他正常往下执行 怎么处理这个问题呢 基本数据类型, 不管他 随它去 引用数据类型, CAS的时候通过Version 去处理</p><h1 id="一、synchronized" tabindex="-1">一、synchronized <a class="header-anchor" href="#一、synchronized" aria-label="Permalink to &quot;一、synchronized&quot;">​</a></h1><h3 id="一、原子性、有序性、可见性" tabindex="-1">一、原子性、有序性、可见性 <a class="header-anchor" href="#一、原子性、有序性、可见性" aria-label="Permalink to &quot;一、原子性、有序性、可见性&quot;">​</a></h3><h4 id="_1-1-原子性" tabindex="-1">1.1 原子性 <a class="header-anchor" href="#_1-1-原子性" aria-label="Permalink to &quot;1.1 原子性&quot;">​</a></h4><p>数据库的事务：ACID</p><p>A：原子性-事务是一个最小的执行的单位，一次事务的多次操作要么都成功，要么都失败。</p><p>并发编程的原子性：一个或多个指令在CPU执行过程中不允许中断的。</p><p>i++;操作是原子性？</p><p>肯定不是：i++操作一共有三个指令</p><p>getfield：从主内存拉取数据到CPU寄存器</p><p>iadd：在寄存器内部对数据进行+1</p><p>putfield：将CPU寄存器中的结果甩到主内存中</p><p>如何保证i++是原子性的。</p><p>锁：synchronized，lock，Atomic（CAS）</p><p>使用lock锁也会有类似的概念，也就是在操作i++的三个指令前，先基于AQS成功修改state后才可以操作</p><p>使用synchronized和lock锁时，可能会触发将线程挂起的操作，而这种操作会触发内核态和用户态的切换，从而导致消耗资源。</p><p>CAS方式就相对synchronized和lock锁的效率更高（也说不定），因为CAS不会触发线程挂起操作！</p><p>CAS：compare and swap</p><p>线程基于CAS修改数据的方式：先获取主内存数据，在修改之前，先比较数据是否一致，如果一致修改主内存数据，如果不一致，放弃这次修改</p><p>CAS就是比较和交换，而比较和交换是一个原子操作</p><p>CAS在Java层面就是Unsafe类中提供的一个native方法，这个方法只提供了CAS成功返回true，失败返回false，如果需要重试策略，自己实现！</p><p>CAS问题：</p><ul><li><p>CAS只能对一个变量的修改实现原子性。</p></li><li><p>CAS存在ABA问题。</p></li><li><p>A线程修改主内存数据从1~2，卡在了获取1之后。</p></li><li><p>B线程修改主内存数据从1~2，完成。</p></li><li><p>C线程修改主内存数据从2~1，完成。</p></li><li><p>A线程执行CAS操作，发现主内存是1，没问题，直接修改</p></li><li><p>解决方案：加版本号</p></li><li><p>在CAS执行次数过多，但是依旧无法实现对数据的修改，CPU会一直调度这个线程，造成对CPU的性能损耗</p></li><li><p>synchronized的实现方式：CAS自旋一定次数后，如果还不成，挂起线程</p></li><li><p>LongAdder的实现方式：当CAS失败后，将操作的值，存储起来，后续一起添加</p></li></ul><p>CAS：在多核情况下，有lock指令保证只有一个线程在执行当前CAS</p><h4 id="_1-2-有序性" tabindex="-1">1.2 有序性 <a class="header-anchor" href="#_1-2-有序性" aria-label="Permalink to &quot;1.2 有序性&quot;">​</a></h4><p>指令在CPU调度执行时，CPU会为了提升执行效率，在不影响结果的前提下，对CPU指令进行重新排序。</p><p>如果不希望CPU对指定进行重排序，怎么办？</p><p>可以对属性追加volatile修饰，就不会对当前属性的操作进行指令重排序。</p><p>什么时候指令重排：满足happens-before原则，即可重排序</p><p>单例模式-DCL双重判断。</p><p>申请内存，初始化，关联是正常顺序，如果CPU对指令重排，可能会造成</p><p>申请内存，关联，初始化，在还没有初始化时，其他线程来获取数据，导致获取到的数据虽然有地址引用，但是内部的数据还没初始化，都是默认值，导致使用时，可能出现与预期不符的结果</p><h4 id="_1-3-可见性" tabindex="-1">1.3 可见性 <a class="header-anchor" href="#_1-3-可见性" aria-label="Permalink to &quot;1.3 可见性&quot;">​</a></h4><p>可见性：前面说过CPU在处理时，需要将主内存数据甩到我的寄存机中再执行指令，指向完指令后，需要将寄存器数据扔回到主内存中。倒是寄存器数据同步到主内存是遵循MESI协议的，说人话就是，</p><p>不是每次操作结束就将CPU缓存数据同步到主内存。造成多个线程看到的数据不一样。</p><p>volatile每次操作后，立即同步数据到主内存。</p><p>synchronized，触发同步数据到主内存。</p><p>final，也可以解决可见性问题。</p><h3 id="二、synchronized使用" tabindex="-1">二、synchronized使用 <a class="header-anchor" href="#二、synchronized使用" aria-label="Permalink to &quot;二、synchronized使用&quot;">​</a></h3><h4 id="_2-1-synchronized的使用方式" tabindex="-1">2.1 synchronized的使用方式 <a class="header-anchor" href="#_2-1-synchronized的使用方式" aria-label="Permalink to &quot;2.1 synchronized的使用方式&quot;">​</a></h4><p>synchronized方法</p><p>synchronized代码块</p><p>类锁和对象锁：</p><p>类锁：基础当前类的Class加锁</p><p>对象锁：基于this对象加锁</p><p>synchronized是互斥锁，每个线程获取synchronized时，基于synchronized绑定的对象去获取锁！</p><p><strong>synchronized锁是基于对象实现的！</strong></p><p>synchronized是如何基于对象实现的互斥锁，先了解对象再内存中是如何存储的。</p><p>在Java中查看。</p><p>导入依赖：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>&lt;dependency&gt;</span></span>
...
<span class="line"><span>&lt;/dependency&gt;</span></span></code></pre></div><p>查看对象信息</p><p>初始化的对象是无锁状态</p><h3 id="三、synchronized锁升级" tabindex="-1">三、synchronized锁升级 <a class="header-anchor" href="#三、synchronized锁升级" aria-label="Permalink to &quot;三、synchronized锁升级&quot;">​</a></h3><p>synchronized在jdk1.6之前，一直是重量级锁：只要线程获取锁资源失败，直接挂起线程（用户-内核）</p><p>在jdk1.6之前synchronized效率贼低，再加上Doug Lea推出了ReentrantLock，效率比synchronized快多了，导致JDK团队不得不在jdk1.6将synchronized做优化</p><p>锁升级：</p><ul><li><p>无锁状态、匿名偏向状态：没有线程拿锁。</p></li><li><p>偏向锁状态：没有线程的竞争，只有一个线程再获取锁资源。<br><strong>线程竞争锁资源时，发现当前synchronized没有线程占用锁资源，并且锁是偏向锁，使用CAS的方式，设置o的线程ID为当前线程，获取到锁资源，下次当前线程再次获取时，只需要判断是偏向锁，并且线程ID是当前线程ID即可，直接获得到锁资源。</strong></p></li><li><p>轻量级锁：偏向锁出现竞争时，会升级到轻量级锁（触发偏向锁撤销）。<br><strong>轻量级锁的状态下，线程会基于CAS的方式，尝试获取锁资源，CAS的次数是基于自适应自旋锁实现的，JVM会自动的基于上一次获取锁是否成功，来决定这次获取锁资源要CAS多少次。</strong></p></li><li><p>重量级锁：轻量级锁CAS一段次数后，没有拿到锁资源，升级为重量级锁（其实CAS操作是在重量级锁时执行的）。<br><strong>重量级锁就是线程拿不到锁，就挂起。</strong></p></li></ul><p>偏向锁是延迟开启的，并且在开启偏向锁之后，默认不存在无锁状态，只存在匿名偏向synchronized因为不存在从重量级锁降级到偏向或者是轻量。</p><p>synchronized在偏向锁升级到轻量锁时，会涉及到偏向锁撤销，需要等到一个安全点，stw，才可以撤销，并发偏向锁撤销比较消耗资源<br> 在程序启动时，偏向锁有一个延迟开启的操作，因为项目启动时，ClassLoader会加载.class文件，这里会涉及到synchronized操作，<br> 为了避免启动时，涉及到偏向锁撤销，导致启动效率变慢，所以程序启动时，默认不是开启偏向锁的。<br> 如果在开启偏向锁的情况下，查看对象，默认对象是匿名偏向。</p><p>编译器优化的结果，出现了下列效果</p><p>锁消除：线程在执行一段synchronized代码块时，发现没有共享数据的操作，自动帮你把synchronized去掉。</p><p>锁膨胀：在一个多次循环的操作中频繁的获取和释放锁资源，synchronized在编译时，可能会优化到循环外部。</p><h3 id="四、synchronized-objectmonitor" tabindex="-1">四、synchronized-ObjectMonitor <a class="header-anchor" href="#四、synchronized-objectmonitor" aria-label="Permalink to &quot;四、synchronized-ObjectMonitor&quot;">​</a></h3><p>涉及ObjectMonitor一般是到达了重量级锁才会涉及到。</p><p>在到达重量级锁之后，重量级锁的指针会指向ObjectMonitor对象。</p><p><a href="http://hg.openjdk.java.net/jdk8u/jdk8u/hotspot/file/69087d08d473/src/share/vm/runtime/objectMonitor.hpp" target="_blank" rel="noreferrer">http://hg.openjdk.java.net/jdk8u/jdk8u/hotspot/file/69087d08d473/src/share/vm/runtime/objectMonitor.hpp</a></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>  ObjectMonitor() {</span></span>
<span class="line"><span>    _header       = NULL;</span></span>
<span class="line"><span>    _count        = 0;     // 抢占锁资源的线程个数</span></span>
<span class="line"><span>    _waiters      = 0,     // 调用wait的线程个数。</span></span>
<span class="line"><span>    _recursions   = 0;     // 可重入锁标记，</span></span>
<span class="line"><span>    _object       = NULL; </span></span>
<span class="line"><span>    _owner        = NULL;  // 持有锁的线程</span></span>
<span class="line"><span>    _WaitSet      = NULL;  // wait的线程  （双向链表）</span></span>
<span class="line"><span>    _WaitSetLock  = 0 ;</span></span>
<span class="line"><span>    _Responsible  = NULL ;</span></span>
<span class="line"><span>    _succ         = NULL ;  // 假定的继承人（锁释放后，被唤醒的线程，有可能拿到锁资源）</span></span>
<span class="line"><span>    _cxq          = NULL ;  // 挂起线程存放的位置。（单向链表）</span></span>
<span class="line"><span>    FreeNext      = NULL ;</span></span>
<span class="line"><span>    _EntryList    = NULL ;  // _cxq会在一定的机制下，将_cxq里的等待线程扔到当前_EntryList里。  （双向链表）</span></span>
...
<span class="line"><span>  }</span></span></code></pre></div><p>偏向锁会降级到无锁状态嘛？怎么降？</p><p>会，当偏向锁状态下，获取当前对象的hashcode值，会因为对象头空间无法存储hashcode，导致降级到无锁状态。</p><h1 id="二、reentrantlock源码" tabindex="-1">二、<strong>ReentrantLock源码</strong> <a class="header-anchor" href="#二、reentrantlock源码" aria-label="Permalink to &quot;二、**ReentrantLock源码**&quot;">​</a></h1><h3 id="一、reentrantlock介绍" tabindex="-1"><strong>一、ReentrantLock介绍</strong> <a class="header-anchor" href="#一、reentrantlock介绍" aria-label="Permalink to &quot;**一、ReentrantLock介绍**&quot;">​</a></h3><p>Java中提供锁，一般就是synchronized和lock锁，ReentrantLock是互斥锁，跟synchronized一样。</p><p>如果竞争比较激烈，推荐lock锁，效率更高</p><p>如果几乎没有竞争，推荐synchronized</p><p>原因：synchronized只有锁升级，当升级到重量级锁后，无法降级到轻量级、偏向锁。</p><p>lock锁的使用相对synchronized成本更高。</p><p>synchronized是非公平锁，lock是公平+非公平锁</p><p>lock锁提供的功能更完善，lock可以使用tryLock指定等待锁的时间</p><p>lock锁还提供了lockInterruptibly允许线程在获取锁的期间被中断。</p><p>synchronized基于对象实现，lock锁基于AQS+CAS实现</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public static void main(String[] args) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h3 id="二、reentrantlock的lock方法源码" tabindex="-1">二、<strong>ReentrantLock的lock方法源码</strong> <a class="header-anchor" href="#二、reentrantlock的lock方法源码" aria-label="Permalink to &quot;二、**ReentrantLock的lock方法源码**&quot;">​</a></h3><p>清楚lock方法是如何实现让当前线程获取到锁资源（什么效果算是拿到了锁资源）</p><p>非公平锁：上来先尝试将state从0修改为1，如果成功，代表获取锁资源。如果没有成功，调用acquire</p><p>公平锁：调用acquire</p><p>state是个什么鬼？</p><p>state是AQS中的一个由volatile修饰的int类型变量，多个线程会通过CAS的方式修改state，在并发情况下，只会有一个线程成功的修改state（从0~1）</p><p>如果线程修改state失败怎么办？</p><p>如果线程没有拿到锁资源，会到AQS的双向链表中排队等待（在期间，线程可能会挂起）</p><p>AQS的双向链表（队列）是个啥？</p><p>AQS中的双向链表是基于内部类Node在维护，Node中包含prev，next，thread属性，并且在AQS中还有两个属性，分别是head，tail。</p><p>画一下AQS的核心</p><p>公平&amp;非公平的方法源码</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>// 公平锁的sync的lock方法</span></span>
<span class="line"><span>final void lock() {</span></span>
<span class="line"><span>    acquire(1);</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>// 非公平锁的sync的lock方法</span></span>
<span class="line"><span>final void lock() {</span></span>
...
<span class="line"><span>        acquire(1);</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="三、reentrantlock的acquire方法源码" tabindex="-1">三、<strong>ReentrantLock的acquire方法源码</strong> <a class="header-anchor" href="#三、reentrantlock的acquire方法源码" aria-label="Permalink to &quot;三、**ReentrantLock的acquire方法源码**&quot;">​</a></h3><p>acquire是一个业务方法，里面并没有实际的业务处理，都是在调用其他方法</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">// 核心acquire     arg = 1</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //1. 调用tryAcquire方法：尝试获取锁资源（非公平、公平），拿到锁资源，返回true，直接结束方法。 没有拿到锁资源，</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //   需要执行&amp;&amp;后面的方法</span></span>
<span class="line"></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //2. 当没有获取锁资源后，会先调用addWaiter：会将没有获取到锁资源的线程封装为Node对象，</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //   并且插入到AQS的队列的末尾，并且作为tail</span></span>
<span class="line"></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //3. 继续调用acquireQueued方法，查看当前排队的Node是否在队列的前面，如果在前面（head的next），尝试获取锁资源</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    //   如果没在前面，尝试将线程挂起，阻塞起来！</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">}</span></span></code></pre></div><h3 id="四、reentrantlock的tryacquire方法" tabindex="-1">四、<strong>ReentrantLock的tryAcquire方法</strong> <a class="header-anchor" href="#四、reentrantlock的tryacquire方法" aria-label="Permalink to &quot;四、**ReentrantLock的tryAcquire方法**&quot;">​</a></h3><p>tryAcquire分为公平和非公平两种、</p><p>tryAcquire主要做了两件事：</p><ul><li><p>如果state为0，尝试获取锁资源</p></li><li><p>如果state不为0，看一下是不是锁重入操作</p></li></ul><p>非公平：</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">// 非公平锁实现！</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 拿到当前线程！</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 拿到AQS的state</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 如果state == 0，说明没有线程占用着当前的锁资源</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 没人占用锁资源，我直接抢一波（不管有没有线程在排队）</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            // 将当前占用这个互斥锁的线程属性设置为当前线程</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            // 返回true，拿锁成功</span></span>
<span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">            return</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> true</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">;</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">        }</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">    }</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 当前state != 0，说明有线程占用着锁资源</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 判断拿着锁的线程是不是当前线程（锁重入）</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 将state再次+1</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 锁重入是否超过最大限制</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 抛出error</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 将值设置给state</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">}</span></span></code></pre></div><p>公平锁：</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">// 公平锁实现</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 拿到AQS的state</span></span>
<span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">    int</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> c </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">=</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;"> getState</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">();</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    // 阿巴阿巴~~~~</span></span>
<span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">    if</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> (c </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">==</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> 0</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">) {</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">        // 判断是否有线程在排队，如果有线程排队，返回true，配上前面的!，那会直接不执行返回最外层的false</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            // 如果没有线程排队，直接CAS尝试获取锁资源</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">    }</span></span>
<span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">    return</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> false</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">;</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">}</span></span></code></pre></div><h3 id="五、reentrantlock的addwaiter方法" tabindex="-1">五、<strong>ReentrantLock的addWaiter方法</strong> <a class="header-anchor" href="#五、reentrantlock的addwaiter方法" aria-label="Permalink to &quot;五、**ReentrantLock的addWaiter方法**&quot;">​</a></h3><p>在获取锁资源失败后，需要将当前线程封装为Node对象，并且插入到AQS队列的末尾</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>// 将当前线程封装为Node对象，并且插入到AQS队列的末尾</span></span>
<span class="line"><span>private Node addWaiter(Node mode) {</span></span>
<span class="line"><span>    // 将当前线程封装为Node对象，mode为null，代表互斥锁</span></span>
<span class="line"><span>    Node node = new Node(Thread.currentThread(), mode);</span></span>
<span class="line"><span>    // pred是tail节点</span></span>
<span class="line"><span>    Node pred = tail;</span></span>
<span class="line"><span>    // 如果pred不为null，有线程正在排队</span></span>
<span class="line"><span>    if (pred != null) {</span></span>
<span class="line"><span>        // 将当前节点的prev，指定tail尾节点</span></span>
<span class="line"><span>        node.prev = pred;</span></span>
<span class="line"><span>        // 以CAS的方式，将当前节点变为tail节点</span></span>
<span class="line"><span>        if (compareAndSetTail(pred, node)) {</span></span>
<span class="line"><span>            // 之前的tail的next指向当前节点</span></span>
<span class="line"><span>            pred.next = node;</span></span>
<span class="line"><span>            return node;</span></span>
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    // 添加的流程为，  自己prev指向、tail指向自己、前节点next指向我</span></span>
<span class="line"><span>    // 如果上述方式，CAS操作失败，导致加入到AQS末尾失败，如果失败，就基于enq的方式添加到AQS队列</span></span>
<span class="line"><span>    enq(node);</span></span>
<span class="line"><span>    return node;</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>// enq，无论怎样都添加进入</span></span>
<span class="line"><span>private Node enq(final Node node) {</span></span>
<span class="line"><span>    for (;;) {</span></span>
<span class="line"><span>        // 拿到tail</span></span>
<span class="line"><span>        Node t = tail;</span></span>
<span class="line"><span>        // 如果tail为null，说明当前没有Node在队列中</span></span>
<span class="line"><span>        if (t == null) { </span></span>
<span class="line"><span>            // 创建一个新的Node作为head，并且将tail和head指向一个Node</span></span>
<span class="line"><span>            if (compareAndSetHead(new Node()))</span></span>
<span class="line"><span>                tail = head;</span></span>
<span class="line"><span>        } else {</span></span>
<span class="line"><span>            // 和上述代码一致！</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="六、reentrantlock的acquirequeued" tabindex="-1">六、<strong>ReentrantLock的acquireQueued</strong> <a class="header-anchor" href="#六、reentrantlock的acquirequeued" aria-label="Permalink to &quot;六、**ReentrantLock的acquireQueued**&quot;">​</a></h3><p>acquireQueued方法会查看当前排队的Node是否是head的next，如果是，尝试获取锁资源，如果不是或者获取锁资源失败那么就尝试将当前Node的线程挂起（unsafe.park()）</p><p>在挂起线程前，需要确认当前节点的上一个节点的状态必须是小于等于0，</p><p>如果为1，代表是取消的节点，不能挂起</p><p>如果为-1，代表挂起当前线程</p><p>如果为-2，-3，需要将状态改为-1之后，才能挂起当前线程</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>// acquireQueued方法</span></span>
<span class="line"><span>// 查看当前排队的Node是否是head的next，</span></span>
<span class="line"><span>// 如果是，尝试获取锁资源，</span></span>
<span class="line"><span>// 如果不是或者获取锁资源失败那么就尝试将当前Node的线程挂起（unsafe.park()）</span></span>
...
<span class="line"><span>    try {</span></span>
<span class="line"><span>        // 循环走起</span></span>
<span class="line"><span>        for (;;) {</span></span>
<span class="line"><span>            // 拿到上一个节点</span></span>
<span class="line"><span>            final Node p = node.predecessor();</span></span>
<span class="line"><span>            if (p == head &amp;&amp; // 说明当前节点是head的next</span></span>
<span class="line"><span>                tryAcquire(arg)) { // 竞争锁资源，成功：true，失败：false</span></span>
<span class="line"><span>                // 进来说明拿到锁资源成功</span></span>
<span class="line"><span>                // 将当前节点置位head，thread和prev属性置位null</span></span>
<span class="line"><span>                setHead(node);</span></span>
<span class="line"><span>                // 帮助快速GC</span></span>
<span class="line"><span>                p.next = null; </span></span>
<span class="line"><span>                // 设置获取锁资源成功</span></span>
<span class="line"><span>                failed = false;</span></span>
<span class="line"><span>                // 不管线程中断。</span></span>
<span class="line"><span>                return interrupted;</span></span>
<span class="line"><span>            }</span></span>
<span class="line"><span>            // 如果不是或者获取锁资源失败,尝试将线程挂起</span></span>
<span class="line"><span>            // 第一个事情，当前节点的上一个节点的状态正常！</span></span>
<span class="line"><span>            // 第二个事情，挂起线程</span></span>
<span class="line"><span>            if (shouldParkAfterFailedAcquire(p, node) &amp;&amp;</span></span>
<span class="line"><span>				// 通过LockSupport将当前线程挂起</span></span>
...
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>// 确保上一个节点状态是正确的</span></span>
<span class="line"><span>private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {</span></span>
<span class="line"><span>    // 拿到上一个节点的状态</span></span>
<span class="line"><span>    int ws = pred.waitStatus;</span></span>
<span class="line"><span>    // 如果上一个节点为 -1</span></span>
<span class="line"><span>    if (ws == Node.SIGNAL)</span></span>
<span class="line"><span>        // 返回true，挂起线程</span></span>
<span class="line"><span>        return true;</span></span>
<span class="line"><span>    // 如果上一个节点是取消状态</span></span>
<span class="line"><span>    if (ws &gt; 0) {</span></span>
<span class="line"><span>        // 循环往前找，找到一个状态小于等于0的节点</span></span>
<span class="line"><span>        do {</span></span>
<span class="line"><span>            node.prev = pred = pred.prev;</span></span>
<span class="line"><span>        } while (pred.waitStatus &gt; 0);</span></span>
<span class="line"><span>        pred.next = node;</span></span>
<span class="line"><span>    } else {</span></span>
<span class="line"><span>        // 将小于等于0的节点状态该为-1</span></span>
<span class="line"><span>        compareAndSetWaitStatus(pred, ws, Node.SIGNAL);</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    return false;</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="七、reentrantlock的unlock方法" tabindex="-1">七、<strong>ReentrantLock的unlock方法</strong> <a class="header-anchor" href="#七、reentrantlock的unlock方法" aria-label="Permalink to &quot;七、**ReentrantLock的unlock方法**&quot;">​</a></h3><p>释放锁资源：</p><ul><li><p>将state-1。</p></li><li><p>如果state减为0了，唤醒在队列中排队的Node。（一定唤醒离head最近的）</p></li></ul><p>释放锁不分公平和非公平，就一个方法。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>// 真正释放锁资源的方法</span></span>
<span class="line"><span>public final boolean release(int arg) {</span></span>
<span class="line"><span>    // 核心的释放锁资源方法</span></span>
<span class="line"><span>    if (tryRelease(arg)) {</span></span>
<span class="line"><span>        // 释放锁资源释放干净了。  （state == 0）</span></span>
<span class="line"><span>        Node h = head;</span></span>
<span class="line"><span>        // 如果头节点不为null，并且头节点的状态不为0，唤醒排队的线程</span></span>
<span class="line"><span>        if (h != null &amp;&amp; h.waitStatus != 0)、</span></span>
<span class="line"><span>            // 唤醒线程</span></span>
<span class="line"><span>            unparkSuccessor(h);</span></span>
<span class="line"><span>        return true;</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    // 释放锁成功，但是state != 0</span></span>
<span class="line"><span>    return false;</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span>// 核心的释放锁资源方法</span></span>
...
<span class="line"><span>    // 如果释放锁的线程不是占用锁的线程，抛异常</span></span>
...
<span class="line"><span>    // 如果头节点状态小于0，换为0</span></span>
<span class="line"><span>    if (ws &lt; 0)</span></span>
<span class="line"><span>        compareAndSetWaitStatus(node, ws, 0);</span></span>
<span class="line"><span>    // 拿到当前节点的next</span></span>
<span class="line"><span>    Node s = node.next;</span></span>
<span class="line"><span>    // 如果s == null ，或者s的状态为1</span></span>
<span class="line"><span>    if (s == null || s.waitStatus &gt; 0) {</span></span>
<span class="line"><span>        // next节点不需要唤醒，需要唤醒next的next</span></span>
<span class="line"><span>        s = null;</span></span>
<span class="line"><span>        // 从尾部往前找，找到状态正常的节点。（小于等于0代表正常状态）</span></span>
